...
if ! rsync -rlD --no-times --no-perms --no-owner --no-group --checksum "$LOCAL_ARCHIVE" "$DEST_DIR/"; then
    log "ERROR: rsync to NAS failed for $APP"
    exit 1
fi
...
log "Backup of '$APP' complete."#!/bin/bash
#
# backup-app.sh — Generic backup script: tar a data directory and push it to
# an NFS-mounted NAS, with retention pruning, logging, and locking.
#
...
# ---------- Copy to NAS ----------
log "Copying archive to NAS: $DEST_DIR"
if ! rsync -a --checksum "$LOCAL_ARCHIVE" "$DEST_DIR/"; then
    log "ERROR: rsync to NAS failed for $APP"
    exit 1
fi
